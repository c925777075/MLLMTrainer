...
from src.dataset.mllm_dataset.rope2d import get_rope_index_25, get_rope_index_2, get_rope_index_3
from transformers import AutoProcessor, AutoTokenizer
from src.dataset.root import DATASETS
from src.dataset.mllm_dataset.pack_data import pack_all_data
...
class Qwen3VL_MLLMDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, cfg):
        super(Qwen3VL_MLLMDataset, self).__init__()
        data_args = cfg
        dataset_list = data_args.dataset_use
        ...
        if data_args.data_packing:
            list_data_dict = pack_all_data(data_args=data_args, datasets=dataset_list, pack_length=data_args.model_max_length)
        else:
            list_data_dict = []
            for data in dataset_list:
                file_format = data["meta_file"].split(".")[-1]
                if file_format == "jsonl":
                    annotations = read_jsonl(data["meta_file"])
                else:
                    annotations = json.load(open(data["meta_file"], "r"))
        ...
        processor = AutoProcessor.from_pretrained(data_args.tokenizer)
    ...
    def collator(self):
        if self.data_args.data_packing:
            return FlattenedDataCollatorForSupervisedDataset(self.tokenizer)
        else:
...
def make_supervised_data_module(processor, data_args) -> Dict:
    ...
    train_dataset = Qwen3VL_MLLMDataset(processor, data_args=data_args)
